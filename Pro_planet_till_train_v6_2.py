# v6.2 Spt 02 2022 final model for 1dcnn with two classes (CT+RT, MT+NT)
"""
...
if __name__ == "__main__":
    ...
    model_name = RESULT_DIR+name_prefix_no_soket+'.cnn1d.model' +'.'+ str(IN_DEPTH) + '.h5'
    ...
    if not os.path.isfile(model_name):  
        ...
        train_xx    = np.mean(train_x, axis=(2,3))
        print(train_xx.shape)
        validate_xx = np.mean(validate_x, axis=(2,3))
        test_xx     = np.mean(test_x, axis=(2,3))
    ##*********************************************************************************
    ...
    with strategy.scope():
        ...
        if os.path.isfile(model_name):
            ...
            model=cnn3d_model.build_1dcnn(IN_DEPTH, IMG_BANDS, 2)
    ...
    accuracy,classes = test_helper.test_accuacy_1dcnn(model, test_xx, test_y, IN_DEPTH, BATCH_SIZE)
    print ('{:0.4f}'.format(accuracy) )
    test_out = np.concatenate((out_arr, np.expand_dims(classes, axis=1)), axis=1)
    out_test_file = './'+'planet.test.cnn1d.v6.2'+'.epoch'+str(EPOCHS)+'.batch'+str(BATCH_SIZE)+'.M'+str(METHOD) +'.step' + str(step) + '.'+str(IN_DEPTH) + '.csv'

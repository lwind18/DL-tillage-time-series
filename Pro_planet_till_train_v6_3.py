# v6.3 Spt 02 2022 final model for cnnlstm with two classes (CT+RT, MT+NT)
"""
...
if __name__ == "__main__":
    ...
    model_name = RESULT_DIR+name_prefix_no_soket+'.cnnlstm.model' +'.'+ str(IN_DEPTH) + '.h5'
    ...
    print(total_x.shape) 
    ## fix bug of reshape by using np.moveaxis to not move pixel values 06/21/2022
    total_xx = np.moveaxis(total_x,3,1)     # change shape to (total_n, 365, 32,32,4) 
   
    ##*********************************************************************************
    ## prepare train and testing data    
    if not os.path.isfile(model_name):  
        train_x, train_y, validate_x, validate_y, test_x, test_y, out_arr = data_process.read_planet_train_all (total_xx, label_y, split=0.3, petg=0.04)               
    ##*********************************************************************************
    ## shuffle before batch -> 
    if not os.path.isfile(model_name):
        train_n = train_x.shape[0]          
        with tf.device("/cpu:0"): 
            train_dataset = tf.data.Dataset.from_tensor_slices( (train_x, train_y)).shuffle(train_n+1).batch(BATCH_SIZE, drop_remainder=True)  
            validation_dataset = tf.data.Dataset.from_tensor_slices( (validate_x, validate_y)).batch(BATCH_SIZE, drop_remainder=True)          
            del train_x,train_y
            gc.collect() 
    ...
    with strategy.scope():
        ...
        if os.path.isfile(model_name):
            ...
            importlib.reload(cnnlstm_model)            
            model=cnnlstm_model.build_cnnlstm0(IMG_HEIGHT, IMG_WIDTH, IN_DEPTH, IMG_BANDS, 2)
    ...
    accuracy,classes = test_helper.test_accuacy_cnnlstm(model, test_x, test_y, IN_DEPTH, BATCH_SIZE)
    print ('{:0.4f}'.format(accuracy) )
    test_out = np.concatenate((out_arr, np.expand_dims(classes, axis=1)), axis=1)
    out_test_file = './'+'planet.test.cnnlstm.v6.3'+'.epoch'+str(EPOCHS)+'.batch'+str(BATCH_SIZE)+'.M'+str(METHOD) +'.step' + str(step) + '.'+str(IN_DEPTH) + '.csv'

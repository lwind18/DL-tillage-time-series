# v6.8 Spt 02 2022 final model for 3dcnn (but with 4*4 input) with two classes (CT+RT, MT+NT) 
"""
...
if __name__ == "__main__":
    ...
    model_name = RESULT_DIR+name_prefix_no_soket+'.3dcnn.model' +'.'+ str(IN_DEPTH) + '.h5'
    ...
    total_xx = data_process.slice_x (x_n, IN_DEPTH)
    print(total_xx.shape)   
    total_x = data_process.get_new_hw(total_xx, IN_DEPTH, 8,8)
    print(total_x.shape)
    ##*********************************************************************************
    ...
    with strategy.scope():
        ...
        if os.path.isfile(model_name):
            ...
            model=cnn3d_model.build_3dcnn4(4, 4, IN_DEPTH, IMG_BANDS, 2)
            logging.info(model.summary())   
#            logging.info(tf.keras.utils.plot_model(model,show_shapes=True))    
    ...
    accuracy,classes = test_helper.test_accuacy_3dcnn4(model, test_x, test_y, IN_DEPTH, BATCH_SIZE)
    print ('{:0.4f}'.format(accuracy) )
    test_out = np.concatenate((out_arr, np.expand_dims(classes, axis=1)), axis=1)
    out_test_file = './'+'planet.test.3dcnn.v6.8'+'.epoch'+str(EPOCHS)+'.batch'+str(BATCH_SIZE)+'.M'+str(METHOD) +'.step' + str(step) + '.'+str(IN_DEPTH) + '.csv'

# v6.0 Spt 02 2022 final model for 3dcnn with two classes (CT+RT, MT+NT)
"""
...
if __name__ == "__main__":
    ...
    with strategy.scope():
        if os.path.isfile(model_name):
            ...
            model=cnn3d_model.build_3dcnn1(IMG_HEIGHT, IMG_WIDTH, IN_DEPTH, IMG_BANDS, 2)
...
#            logging.info(tf.keras.utils.plot_model(model,show_shapes=True))    
    ...
    accuracy,classes = test_helper.test_accuacy_3dcnn(model, test_x, test_y, IN_DEPTH, BATCH_SIZE)
    print ('{:0.4f}'.format(accuracy) )
    # combine out_arr (test_n,2) and classesi (test_n,) >>> get csv as (test_n,3)
    test_out = np.concatenate((out_arr, np.expand_dims(classes, axis=1)), axis=1)
    out_test_file = './'+'planet.test.3dcnn.v6.0'+'.epoch'+str(EPOCHS)+'.batch'+str(BATCH_SIZE)+'.M'+str(METHOD) +'.step' + str(step) + '.'+str(IN_DEPTH) + '.csv'
